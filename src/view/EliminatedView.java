...
public class EliminatedView {
    ...
    public EliminatedView(Jeu jeu){
        ...
        if (jeu.getCoupe()!=null){
            titleText = new Text("You Were Eliminated !");
            ...
            titleText = new Text("You Lost The League !");
            titleText.setFont(Font.font("Sports World", 60));
            titleText.setStyle(
                "-fx-fill: linear-gradient(from 0% 0% to 100% 100%, #FF0000, #8B0000);" +  // Dégradé rouge clair à rouge foncé
                "-fx-stroke: black;" +                                                   // Contour noir
        ...
        layout.getChildren().addAll( backgroundImage, titleText, restartButton, quitButton);

        // Positionnement
        StackPane.setAlignment(titleText, Pos.TOP_CENTER);
        StackPane.setMargin(titleText, new javafx.geometry.Insets(100, 0, 0, 0));
        StackPane.setMargin(restartButton, new javafx.geometry.Insets(0, 0, 100, 0));
